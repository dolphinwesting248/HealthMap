...
def get_domains():
    """获取 12 个卫生领域的 cid 和 indicatorIds"""
    print("[S5] 获取卫生领域列表...")
...
def main():
    ...
    for i, domain in enumerate(domains):
        print(f"[{i+1}/{len(domains)}] {domain['name']}...")
        df = collect_domain(domain, OUTPUT_DIR)
        summary.append({"domain": domain["name"], "rows": len(df), "indicators": len(domain["indicator_ids"])})
        print(f"  → {len(df)} 条\n")
    ...
    print("采集汇总:")
    for s in summary:
        print(f"  {s['domain']:40s} | {s['rows']:6d} 条 | {s['indicators']:2d} 指标")
    total = sum(s["rows"] for s in summary)
    print(f"  {'总计':40s} | {total:6d} 条")
