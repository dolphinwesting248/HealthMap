...
RAW_DIR = Path(__file__).resolve().parent.parent.parent / "data" / "raw" / "s5_gbd"


def clean_s5():
    print("\n[S5] 清洗省级卫生数据...")

    csv_files = sorted(RAW_DIR.glob("nbs_health_*.csv"))
    if not csv_files:
        log("未找到原始数据文件")
        return
    ...
    for f in csv_files:
        ...
        domain = f.stem.replace("nbs_health_", "")
        df["domain"] = domain
        all_dfs.append(df)
    ...
    # 4. 缺失值处理
    total = len(df)
    na_value = df["value_num"].isna().sum()
    na_pct = na_value / total * 100 if total > 0 else 0

    # 区分: 空字符串 = 数据未发布(保留NA) vs 非数字 = 数据异常
    ...
    out = CLEANED_DIR / "s5_health_provincial.csv"
